...
static void bank3_init (void)	// for s1d13521
{
	SMBCR3_REG |=  ((1<<15)|(1<<7));		//dsf
//	SMBCR3_REG |=  ((1<<2)|(1<<0));		//SMWAIT active High, Read Byte Lane Enabl	    WS1?
	SMBCR3_REG &=  ~(1<<2);				//SMWAIT deactive because s1d13521 too slow
	SMBCR3_REG &= ~((3<<20)|(3<<12));	//SMADDRVALID = always High when Read/Write
	SMBCR3_REG &= ~(3<<4);			//Clear Memory Width
	SMBCR3_REG |=  (1<<4);			//Memory Width = 16bit
}

static void usb_set_2host(void)
{
	writel(0, USB_PHYPWR);
	writel((1<<4), PWRCFG);
	writel((0<<2)|(1<<1)|(1<<0), USB_RSTCON);
	writel((0<<2)|(0<<1)|(0<<0), USB_RSTCON);
	writel((0<<3)|(0<<2)|(0<<1)|(1<<0), USB_PHYCTRL);
	writel((0<<31)|(0<<2)|(1<<1)|(0<<0), USB_CLKCON);
}
...
int board_init(void)
{
	...
	tmp = readl(GPBDAT);	// set GPB4 as high to disable beep
	tmp |= 0x10;
	writel(tmp, GPBDAT);
	tmp = readl(GPBCON);	// set GPB4 as output BEEP control port
	tmp &= (~0x300);
	tmp |= 0x100;
	writel(tmp, GPBCON);

    tmp = readl(GPDDAT);	// turn on led
    tmp |= 0x100;
    writel(tmp, GPDDAT);
    tmp = readl(GPDCON);    // set GPB4 as output BEEP control port
    tmp &= (~0x30000);
    tmp |= 0x10000;
    writel(tmp, GPDCON);
