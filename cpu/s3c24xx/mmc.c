...
static ulong sd_movi_write(ulong start, lbaint_t blkcnt, void *src)
{
	return mmc_bwrite(0, start, blkcnt, src);
}

static ulong sd_movi_read(ulong start, lbaint_t blkcnt, void *dst)
{
...
{
	sd_movi_write(start_blk, blknum, (void *)addr);
}

void movi_read (uint addr, uint start_blk, uint blknum)
{
	sd_movi_read(start_blk, blknum, (void *)addr);
}
...
int mmc_send_app_op_cond(struct mmc *host)
{
	...
	do {
		 ...
		 * specified.
		 */
		cmd.arg = host->voltages & 0xff8000;
...
int mmc_change_freq(struct mmc *mmc)
{
	...
	if (ext_csd[212] || ext_csd[213] || ext_csd[214] || ext_csd[215])
	{
		mmc->high_capacity = 1;
		#if SD_BOOT_DEBUG
		printf("mmc->high_capacity = 1;line= %d\r\n",__LINE__);
		#endif
	}
...
retry_scr:
	...
	if ((__be32_to_cpu(switch_status[4]) & 0x0f000000) == 0x01000000) {
		...
		mmc->clock = 50000000;
		#if SD_BOOT_DEBUG
		printf("mmc->clock = 50000000;;line= %d\r\n",__LINE__);
		#endif
	} else {
		mmc->clock = 25000000;
	}
...
int mmc_init(struct mmc *host)
{
	...
	mmc_set_bus_width(host, 0 /*MMC_BUS_WIDTH_1*/);
