	...
	if (fifo_count == 5) {
		...
		if (temp==0x1) {
			...
			printf("UploadAddress : 0x%x, UploadSize: 0x%x\n",
				(uint)upload_ptr, (uint)upload_size);
...
//	printf(".");
	printf("download_ptr = 0x%x, download_addr = 0x%x, download_filesize = 0x%x\n", (uint)download_ptr, (uint)download_addr, (uint)download_filesize);
	...
	if (ep3_status & UD_DMA_TOTAL_COUNT_ZERO) {
		...
		if (ep3_status & UD_EP_RX_SUCCESS) {	/* short packet handled */
			...
			printf("download_ptr: 0x%x, Download Address: 0x%x, Download Filesize:0x%x\n",
				(uint)download_ptr, (uint)download_addr, (uint)(download_filesize-10));
...
#else
	USB_PHYCTRL_REG = (0x0<<3)|(0<<2)|(0x1<<1)|(0x0<<0);
#endif
